...
const monthlyKey = () => {
  const d = new Date();
  return `interviewhub_monthly_${d.getFullYear()}-${d.getMonth()}`;
};

export function getMonthlyInterviewCount() {
  try {
    return parseInt(localStorage.getItem(monthlyKey()) || "0", 10);
  } catch {
    return 0;
  }
}

export function incrementMonthlyInterviewCount() {
  try {
